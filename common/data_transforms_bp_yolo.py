...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        labels_coor = sample['label_coor']
        labels_h = sample['label_h']
        filled_labels = np.zeros((self.max_objects, 5), np.float32)
        filled_labels[range(len(labels_coor))[:self.max_objects]] = labels_coor[:self.max_objects]
        return {'image': image, 'label': labels, 'label_coor': torch.from_numpy(filled_labels), 'label_h': labels_h}
...
class ResizeImage(object):
    ...
    def __call__(self, sample):
        image, label = sample['image'], sample['label']
        label_coor = sample['label_coor']
        labels_h = sample['label_h']
        image = cv2.resize(image, self.new_size, interpolation=self.interpolation)
        return {'image': image, 'label': label, 'label_coor': label_coor,'label_h': labels_h}
